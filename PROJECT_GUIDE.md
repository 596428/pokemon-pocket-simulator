...
**프로젝트 경로**: `C:\CodePracticeProject\test_project`
...
#### `auto_test.py`
**용도**: 단일 테스트 자동 실행 (첫 번째 케이스만)
- 빠른 단일 테스트용
- 사용자 입력 없이 자동 진행

#### `test_file_loading.py`
**용도**: 파일 읽기 기능 검증
- DeckList.txt와 TestCase.txt 파싱 테스트
- 데이터 로드 상태 확인
...
### **파일 읽기 오류**
```bash
python test_file_loading.py  # 파일 형식 확인
python debug_deck.py         # 덱 파싱 디버그
```
